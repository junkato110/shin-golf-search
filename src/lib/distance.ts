 ...
 * 直線距離から車での所要時間を概算する。
 *
 * 起点から **必ず高速道路を利用** する前提のベストケース見積り。
 * 早朝・休日深夜などの「ほぼ渋滞しない」状況を想定。
 *
 * - 道路係数 1.2 (高速 IC への乗り入れ/降りる際の最小限の迂回のみ)
 * - 平均車速 90 km/h (高速 100km/h を中心、IC 前後の一般道でわずかに減速)
 *
 * 厳密な計算ではなく、検索時のフィルタ用途に十分な近似値。
 * ナビ no-traffic 値より 5-10% 短めに出ることを許容。
 */
const ROAD_FACTOR = 1.2;
const AVERAGE_SPEED_KMH = 90;
